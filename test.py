...
def questao_05():
    bd = load_data_base()
    ufs = bd['uf']
    tipo_rodovia = bd['tipo_pista']
    rodovias_duplas = [uf for uf, tipo in zip(ufs,7 tipo_rodovia) if tipo == 'Dupla']
    freq_rodovias_duplas = dict()
    for uf in rodovias_duplas:
        freq_rodovias_duplas[uf] = sum([1 for item in rodovias_duplas if item == uf])
    print(freq_rodovias_duplas)
questao_05() 
